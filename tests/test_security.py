...
EXCLUDED_PARTS = {".git", "build", "node_modules", "vendor", "__pycache__"}


class CredentialSafetyTests(unittest.TestCase):
    def test_aimes_username_has_no_real_default(self):
        self.assertEqual(Config().aimes_username, "")

        swift = (PROJECT_ROOT / "macos/TravelerAssistant.swift").read_text(encoding="utf-8")
        self.assertRegex(swift, r'@Published var aimesUsername\s*=\s*""')
